...
@property (nonatomic, assign, readonly) BitLineStyle bStyle;        ///< k线日期
...
- (void)updateKLineTitles:(NSArray<id<KLineAbstract, VolumeAbstract, QueryViewAbstract>> *)kLineArray
{
    ...
    else if (_bStyle == BitLineTypeWeek) {
    ...
    else if (_bStyle == BitLineTypeMonth) {
...
- (void)baseConfigRendererAndLayer
{
    ...
    // 纵轴
    __weak BitLineChart * weakSelf = self;
...
- (void)touchIndexLayer:(UILongPressGestureRecognizer *)recognizer
{
    ...
    if (CGRectContainsPoint(self.greenLineLayer.frame, velocityInScroll)) {
        ...
        if (_kLineIndexIndex > [[BitLineChart kLineIndexLayerClazzName] count] - 1) {
    ...
    if (CGRectContainsPoint(self.volumIndexLayer.frame, velocityInScroll)) {
        ...
        if (_volumIndexIndex > [[BitLineChart kVolumIndexLayerClazzName] count] - 1) {
...
- (void)updateKLineIndexLayer:(NSInteger)index
{
    runMainThreadWithBlock(^{
        ...
        [self->_kLineIndexLayer removeFromSuperlayer];
        
        self->_kLineIndexIndexName = [BitLineChart kLineIndexLayerClazzName][index];
        Class clazz = NSClassFromString([self->_kLineIndexIndexName stringByAppendingString:@"Layer"]);
        
        self->_kLineIndexLayer = [[clazz alloc] init];
        self->_kLineIndexLayer.frame = self.redLineLayer.frame;
        self->_kLineIndexLayer.gg_width = self.kLineScaler.contentSize.width;
        [self->_kLineIndexLayer setKLineArray:self->_kLineArray];
        self->_kLineIndexLayer.currentKLineWidth = self.kLineScaler.shapeWidth;
        [self.scrollView.layer addSublayer:self->_kLineIndexLayer];
        [self updateSubLayer];
        
        if (self->_indexChangeBlk) { self->_indexChangeBlk([BitLineChart kLineIndexLayerClazzName][index]); }
    });
}
...
- (void)updateVolumIndexLayer:(NSInteger)index
{
    ...
    runMainThreadWithBlock(^{
        ...
        //        [self->_volumIndexLayer removeFromSuperlayer];
        self->_volumIndexIndexName = [BitLineChart kVolumIndexLayerClazzName][index];
        Class clazz = NSClassFromString([self->_volumIndexIndexName stringByAppendingString:@"Layer"]);
        //lyh MAVOLLayer
        self->_volumIndexLayer = [[clazz alloc] init];
        //        self->_volumIndexLayer.frame = self.redVolumLayer.frame;
        //        [self->_volumIndexLayer setKLineArray:self->_kLineArray];
        //        self->_volumIndexLayer.currentKLineWidth = self.kLineScaler.shapeWidth;
        //        [self.scrollView.layer addSublayer:self->_volumIndexLayer];
        [self updateSubLayer];
        
//        if (self->_indexChangeBlk) { self->_indexChangeBlk([BitLineChart kVolumIndexLayerClazzName][index]); }
    });
}
...
- (void)updateSubLayer
{
    ...
    // 更新视图
//    [self updateKLineLayerWithRange:range]; //lyh取消K线
...
- (void)updateGridBackLayerWithRange:(NSRange)range
{
    ...
    if (size.width + self.greenLineLayer.gg_left < self.gg_width) {
        ...
        while (drawX < self.gg_width) {
            ...
            title = date.month == 1 || _bStyle == BitLineTypeMonth ? [date stringWithFormat:@"yyyy/MM"] : title;
...
//    self.greenVolumLayer.hidden = ![self.volumIndexLayer isKindOfClass:[MAVOLLayer class]];
    ...
    //b不要
//    [self.volumIndexLayer getIndexWithRange:range max:&max min:&min]; //获取区间最大值最小值
//    [self.volumIndexLayer updateLayerWithRange:range max:max min:min];//绘制指标层
